import { Box, Modal, CloseButton, Group, Text, Input, Button, Space, Stack } from '@mantine/core';
...
function RepForm({ submit, cancel, defaultValue }: RepFormProps) {
...
export function Reps({ reps, remove, updateRep }: RepsProps) {
    ...
    const lines = useMemo(() => [...reps].reverse().map((rep, idx) => {
        ...
        return (
            <Group key={`rep-${idx}`} justify="flex-end">
                <Text onClick={() => editRep(rep)}>{rep.name} - {formatTime(rep.time)}s</Text>
    ...
    return (<>
        ...
        <Modal opened={opened} onClose={close} title="Edit Rep">
            <RepForm defaultValue={formRep?.name} submit={updateName} cancel={close} />
        </Modal>
    </>);
}
