...
export function getCurrentTime(timer: RunningTimer, systemTime: number): number {
...
export function stopTimer(timer: RunningTimer, systemTime: number): StoppedTimer {
    return {
        time: getCurrentTime(timer, systemTime)
    }
}

export function startTimer(timer: StoppedTimer, systemTime: number): RunningTimer {
    return {
...
export function toggleTimer(timer: Timer, systemTime: number): Timer {
    return isRunning(timer) ? stopTimer(timer, systemTime) : startTimer(timer, systemTime);
}